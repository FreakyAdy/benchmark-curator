...
class TestCLIClean:
    ...
    def test_clean_with_min_tokens(self, tmp_path):
        ...
        result = runner.invoke(
            app, ["clean", str(input_file), "--output", str(output_file), "--min-tokens", "5"]
        )
        assert result.exit_code == 0
...
class TestCLIIntegration:
    ...
    def test_e2e_gsm8k_pipeline(self, mock_download, tmp_path):
        ...
        def mock_download_side_effect(
            benchmark, split=None, output=None, config=None, hf_token=None
        ):
            records = [
                {"question": "What is 2+2?", "answer": "4"},
        ...
        result = runner.invoke(
            app, ["download", "gsm8k", "--split", "train", "--output", str(raw_file)]
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            app,
            ["clean", str(raw_file), "--output", str(clean_file), "--dedupe", "--min-tokens", "5"],
        )
        assert result.exit_code == 0
